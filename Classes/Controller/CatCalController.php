 ...
 ***************************************************************/
  ...
  * CatCal Controller
  */
class CatCalController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	/**
	 * dateRepository
	 * 
	...
	public function listAction(\PfarreNg\Calmedia\Domain\Model\Typ $cat = null) {
		$sec = $this->sectionRepository->findByUid($this->settings['section']['id']);
		if($cat){
			$this->view->assign('dates', $this->dateRepository->listSecDates($sec, $cat));
			$this->view->assign('cat', $cat);
		}else{
			$this->view->assign('dates', $this->dateRepository->listSecDates($sec));
		}
		$this->view->assign('holidays', $this->holidayRepository->getHolidayArray());
		$this->view->assign('types', $this->typRepository->findAll());
		$this->view->assign('section', $sec);
	}
}